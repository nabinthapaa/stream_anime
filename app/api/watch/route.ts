...
export async function GET(req: NextRequest, res: NextResponse) {
  try {
    ...
    if (Object.keys(response).includes("filelions")) {
      ...
      if (!video && Object.keys(response).includes("streamwish"))
        video = await getVideo(response.streamwish);
      return new Response(JSON.stringify({ video, other_source: response }), {
        status: 200,
      });
    }
