...
export const HomeSection:React.FC<HomeSectionProps> = async ({title, link}) => {
return (
    <div className="container mx-auto my-6 space-y-4">
        <div className="flex justify-between px-4">
            <h2 className="font-bold text-2xl">{title}</h2>
            <div>
                <a className="font-bold text-sm hover:underline hover:underline-offset-4" aria-label={"View more " + link} href={`/${link}`}>View More &gt;</a>
            </div>
        </div>
...
const Card = async ({link}:{link:string}) => {
    ...
    return  data?.results && data?.results.map((element, index) => {
                if (index > 5) return;
                    return (
                        <a href={`/info/${element.id}`} aria-label={"Info " + element.name} key={element.id} className="rounded-md p-4  w-fit space-y-1">
                            ...
                            <div>
                            <div title={element.name} className="w-32 md:w-48 line-clamp-1 overflow-hidden overflow-ellipsis">
                                <h3 className="font-md text-lg">{element.name}</h3>
                            </div>
                            <div className="h-4 w-1/2 rounded-full max-w-sm">
                            <p className="font-light text-sm">{element.date ? <><span className="hidden md:inline">Released:</span> {element.date}</> : element.episode}</p>
                            </div>
                            </div>
                        </a>
                        )
