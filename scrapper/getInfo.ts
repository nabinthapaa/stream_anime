...
export async function GetInfo(id: string) {
    try {
        ...
        let genre: string[] = [];
        let plot = anime_details.find('p:nth-child(5)').text().replace(/^Plot Summary:\s*/gm, '');
        let otherNames = anime_details.find('p:nth-child(9)').text().replace(/^Other name:\s*/gm, '');
        let status = anime_details.find('p:nth-child(8)').text().replace(/^Status:\s*/gm, '').replace(/\n/, '').trim();
        let releasedOn = anime_details.find('p:nth-child(7)').text().replace(/^Released:\s*/gm, '');
        anime_details.find('[href^="https://gogoanimehd.io/genre/"]').each((_, el) => {
            genre.push($(el).text().replace(/,\s*/g, ""));
        });
        let type = anime_details.find("[href^='/sub-category/']").text().trim();
        let totalEpisodes = Number($("#episode_page li").last().find('a').attr("ep_end"));
